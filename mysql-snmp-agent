...
sub usage
{
	...
	print "  -b|--heartbeat DB.TABLE  table for maatkit mk-heartbeat \n";
...
GetOptions(\%opt, 'help', 'version|V',
	...
	'm|master','s|slave', 'b|heartbeat=s', 'oid', 'i|refresh=i',
...
my $hb_table   = $opt{b} || '';	# Are you using mk-hearbeat from maatkit?
...
{
	$oids{$regOID.".$i.0"} = { 'name' => $oidname, 'oid' => new NetSNMP::OID($regOID.".$i.0") };
	$i++;
}
	...
	eval
	{
		...
		if ( $chk_slave ) 
		{
			...
			foreach my $row (@$result)
			{
				...
				if ( $hb_table ne '' ) {
					my $row2 = $dbh->selectrow_arrayref(
						"SELECT GREATEST(0, UNIX_TIMESTAMP() - UNIX_TIMESTAMP(ts) - 1)"
						. "FROM $hb_table WHERE id = 1");
		...
		if ( $chk_master && $status{'log_bin'} eq 'ON' ) { # See issue #8
			$result = $dbh->selectall_arrayref("SHOW MASTER LOGS", { Slice => {} });
			foreach my $row (@$result)
			{
				my %newrow = map { lc($_) => $row->{$_} } keys %$row;
...
sub refresh_status {
	...
	if (($now - $global_last_refresh) < $refresh_interval) 
	{
		...
		dolog(LOG_DEBUG, "not refreshing: ".($now - $global_last_refresh). " < $refresh_interval") if ($debugging);
		return;
	}
